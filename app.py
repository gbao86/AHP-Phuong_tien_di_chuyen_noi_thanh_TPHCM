...
app = Flask(__name__)
app.secret_key = 'my_ahp_transport_secret_key_v12_final_final_final_!@#$' # Cập nhật key

NUM_CRITERIA = 5
CRITERIA_NAMES = ["Chi phí", "Thời gian", "Tiện lợi", "An toàn", "Môi trường"]

# Enum cho các loại phương tiện
TRANSPORT_TYPES = {
    'MOTORBIKE': {'id': 'motorbike', 'name': 'Xe máy', 'image': 'motorbike.jpg'},
    'BUS': {'id': 'bus', 'name': 'Xe buýt', 'image': 'bus.jpg'},
    'METRO': {'id': 'metro', 'name': 'Metro/Tàu điện', 'image': 'metro.jpg'},
    'CAR': {'id': 'car', 'name': 'Xe ô tô', 'image': 'car.jpg'},
    'RIDE_HAILING': {'id': 'ride_hailing', 'name': 'Xe công nghệ', 'image': 'ride_hailing.jpg'},
    'OTHER': {'id': 'other', 'name': 'Khác', 'image': 'default.jpg'}
}

def get_transport_image(transport_type):
    """Hàm helper để lấy hình ảnh dựa trên loại phương tiện"""
    return TRANSPORT_TYPES.get(transport_type, TRANSPORT_TYPES['OTHER'])['image']
...
def store_ahp_results_to_db(decision_name, description, results_data_dict, form_data):
...
def index():
    ...
    if request.method == 'POST':
        ...
        session['last_form_data'] = submitted_data
        action = request.form.get('action')
        try:
            if action == 'calculate_criteria':
                criteria_results = _calculate_criteria_logic(request.form)
                ...
                if not criteria_data_for_full_calc or 'weights' not in criteria_data_for_full_calc:
                    ...
                    decision_name_for_db = f"Phương tiện di chuyển (Web) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                    decision_description_for_db = "Kết quả AHP từ trang web."
                    saved_decision_id = store_ahp_results_to_db(decision_name_for_db, decision_description_for_db, ahp_final_results, request.form)
                    if saved_decision_id:
                        flash(f"Đã tính toán và lưu kết quả vào lịch sử (ID: {saved_decision_id}).", "success")
                    else:
                        flash("Tính toán AHP thành công, nhưng có lỗi khi lưu vào lịch sử.", "warning")
                    session.pop('criteria_results', None)
            ...
            flash(f"Lỗi không xác định: {e_gen}", "error")
            criteria_results = session.get('criteria_results', None)
    else:  # GET request
        if 'last_form_data' in session and request.args.get('new_session') != 'true':
            submitted_data = session.get('last_form_data', {})
            criteria_results = session.get('criteria_results', None)
        else:
            session.pop('criteria_results', None)
            session.pop('last_form_data', None)
    ...
    return render_template('index.html', 
                         ...
                         ahp_final_results=ahp_final_results, 
...
def decision_results_page(decision_id):
    ...
    # Thêm thông tin hình ảnh cho phương án tốt nhất
    best_alternative = next((alt for alt in alternatives_from_db if alt.get('rank_value') == 1), None)
    if best_alternative:
        best_alternative['image'] = get_transport_image(best_alternative['alternative_name'])
    ...
    results_for_display = {
        ...
        'best_alternative_image': best_alternative['image'] if best_alternative else 'default.jpg'
    }
...
def _generate_excel_workbook(ahp_data_dict):
    ...
    # Sheet 1: Thông tin tiêu chí
    ws1 = wb.active
    ws1.title = "TieuChi_AHP"
    
    # Tiêu đề và thông tin CR
    ws1.append(["THÔNG TIN TIÊU CHÍ VÀ TRỌNG SỐ"])
    ws1.append([])
    ws1.append(["Tỷ số nhất quán (CR):", ahp_data_dict.get('cr', 'N/A')])
    ws1.append(["Trạng thái:", "Nhất quán" if ahp_data_dict.get('is_consistent') else "Không nhất quán"])
    ws1.append([])

    # Bảng trọng số tiêu chí
    ws1.append(["BẢNG TRỌNG SỐ TIÊU CHÍ"])
    headers = ["Tiêu chí", "Trọng số"]
    ws1.append(headers)
    
    criteria_names = ahp_data_dict.get('criteria_names', [])
    weights = ahp_data_dict.get('weights', [])
    
    for i, (name, weight) in enumerate(zip(criteria_names, weights)):
        ws1.append([name, weight])

    # Ma trận so sánh cặp
    ws1.append([])
    ws1.append(["MA TRẬN SO SÁNH CẶP"])
    comparison_matrix = ahp_data_dict.get('comparison_matrix', [])
    if comparison_matrix:
        # Header cho ma trận
        matrix_header = [""] + criteria_names
        ws1.append(matrix_header)
        
        # Dữ liệu ma trận
        for i, row in enumerate(comparison_matrix):
            matrix_row = [criteria_names[i]] + [format_float(val, 4) for val in row]
            ws1.append(matrix_row)

    # Sheet 2: Thông tin phương án
    ws2 = wb.create_sheet(title="PhuongAn_AHP")
    
    # Tiêu đề
    ws2.append(["THÔNG TIN PHƯƠNG ÁN VÀ ĐIỂM SỐ"])
    ws2.append([])

    # Bảng xếp hạng phương án
    ws2.append(["BẢNG XẾP HẠNG PHƯƠNG ÁN"])
    alt_headers = ["Xếp hạng", "Tên phương án", "Điểm tổng hợp"] + [f"Điểm {name}" for name in criteria_names]
    ws2.append(alt_headers)

    ranked_alternatives = ahp_data_dict.get('ranked_alternatives', [])
    for alt in ranked_alternatives:
        alt_row = [
            alt.get('rank', 'N/A'),
            alt.get('name', 'N/A'),
            format_float(alt.get('overall_score'), 4)
        ]
        # Thêm điểm chi tiết theo từng tiêu chí
        raw_scores = alt.get('raw_scores', [])
        for score in raw_scores:
            alt_row.append(format_float(score, 4) if score is not None else 'N/A')
        ws2.append(alt_row)

    # Định dạng cho cả hai sheet
    for ws in [ws1, ws2]:
        # Định dạng tiêu đề
        for cell in ws[1]:
            cell.font = Font(bold=True, size=12)
            
        # Định dạng headers của bảng
        for row in ws.iter_rows():
            for cell in row:
                if cell.value in headers or cell.value in alt_headers:
...
def export_excel_all():
    try:
        ...
        all_decisions = db_manager.get_all_decisions()
        if not all_decisions:
            flash("Không có quyết định nào để xuất.", "warning")
            return redirect(url_for('history'))

        # Tạo workbook mới
        wb = Workbook()
        ws = wb.active
        ws.title = "Danh Sách Quyết Định"

        # Tạo header
        headers = ["ID", "Tên Quyết Định", "Mô tả", "Ngày Tạo", "CR", "Phương Án Tốt Nhất", "Điểm Số"]
        for col, header in enumerate(headers, 1):
            ws.cell(row=1, column=col, value=header)
            ws.cell(row=1, column=col).font = Font(bold=True)
            ws.cell(row=1, column=col).fill = PatternFill(start_color="E2EFDA", end_color="E2EFDA", fill_type="solid")

        # Thêm dữ liệu
        for row, decision in enumerate(all_decisions, 2):
            # Lấy thông tin chi tiết về phương án tốt nhất
            alternatives = db_manager.get_alternatives_with_scores_for_decision(decision['decision_id'])
            best_alternative = next((alt for alt in alternatives if alt.get('rank_value') == 1), None) if alternatives else None

            ws.cell(row=row, column=1, value=decision['decision_id'])
            ws.cell(row=row, column=2, value=decision.get('decision_name', 'N/A'))
            ws.cell(row=row, column=3, value=decision.get('description', ''))
            ws.cell(row=row, column=4, value=decision['creation_date'].strftime('%d/%m/%Y %H:%M:%S') if decision.get('creation_date') else 'N/A')
            ws.cell(row=row, column=5, value=float(decision['criteria_cr']) if decision.get('criteria_cr') is not None else 'N/A')
            ws.cell(row=row, column=6, value=best_alternative['alternative_name'] if best_alternative else 'N/A')
            ws.cell(row=row, column=7, value=float(best_alternative['overall_score']) if best_alternative and best_alternative.get('overall_score') is not None else 'N/A')

        # Tạo sheet thứ hai cho thông tin chi tiết
        ws2 = wb.create_sheet(title="Chi Tiết Phương Án")
        detail_headers = ["ID QĐ", "Tên QĐ", "Phương Án", "Xếp Hạng", "Điểm Tổng Hợp"]
        for col, header in enumerate(detail_headers, 1):
            ws2.cell(row=1, column=col, value=header)
            ws2.cell(row=1, column=col).font = Font(bold=True)
            ws2.cell(row=1, column=col).fill = PatternFill(start_color="E2EFDA", end_color="E2EFDA", fill_type="solid")

        row_idx = 2
        for decision in all_decisions:
            alternatives = db_manager.get_alternatives_with_scores_for_decision(decision['decision_id'])
            for alt in alternatives:
                ws2.cell(row=row_idx, column=1, value=decision['decision_id'])
                ws2.cell(row=row_idx, column=2, value=decision.get('decision_name', 'N/A'))
                ws2.cell(row=row_idx, column=3, value=alt.get('alternative_name', 'N/A'))
                ws2.cell(row=row_idx, column=4, value=alt.get('rank_value', 'N/A'))
                ws2.cell(row=row_idx, column=5, value=float(alt['overall_score']) if alt.get('overall_score') is not None else 'N/A')
                row_idx += 1

        # Điều chỉnh độ rộng cột cho cả hai sheet
        for ws_adjust in [ws, ws2]:
            for col_idx in range(1, ws_adjust.max_column + 1):
                column_letter = get_column_letter(col_idx)
                max_length = 0
                for row_idx in range(1, ws_adjust.max_row + 1):
                    cell = ws_adjust.cell(row=row_idx, column=col_idx)
                    if cell.value:
                        try:
                            max_length = max(max_length, len(str(cell.value)))
                        except:
                            pass
                adjusted_width = (max_length + 2)
                ws_adjust.column_dimensions[column_letter].width = adjusted_width
        ...
        filename = f"AHP_TatCaQuyetDinh_{timestamp}.xlsx"
...
if __name__ == '__main__':
    print("Chào mừng bạn đến với Ứng dụng Hỗ trợ Ra Quyết Định Lựa Chọn Phương Tiện Di Chuyển Trong Nội Thành TPHCM!")
